...
$attributeString = mkAttrs(new ImmutableArray([
    'height' => new MutableString($height ?? 'false')->match('/^(-?\d+(\/\d+)?(\.\d+)?)$/', default: '1'),
    'rounded' => new MutableString($rounded ?? 'false')->match('/^(false|true)$/', default: 'false'),
    'width' => new MutableString($width ?? 'false')->match('/^(-?\d+(\/\d+)?(\.\d+)?)$/', default: '1'),
]));
